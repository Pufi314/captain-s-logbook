...
const Dashboard = ({ trips, onTripSelect, captainName }) => {
  ...
  const StatCard = ({ title, value, icon: Icon, subtitle, onClick }) => {
    return (
      <div onClick={onClick} className={`bg-white/60 p-3 rounded-lg shadow-sm border border-gray-100 flex items-center gap-3 ${onClick ? 'cursor-pointer hover:shadow-md transition-shadow text-left w-full' : ''}`} {...(onClick ? { role: 'button', tabIndex: 0, onKeyDown: (e) => { if (e.key === 'Enter' || e.key === ' ') onClick(e); } } : {})}>
    ...
    <div className="space-y-6">
      <section>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          <div className="bg-white/60 p-3 rounded-lg shadow-sm border border-gray-100 flex items-center gap-3">
          ...
          <div className="bg-white/60 p-3 rounded-lg shadow-sm border border-gray-100 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm font-bold">Anchor: {((aggregated.kotvaCount / (aggregated.totalOvernights || 1)) * 100).toFixed(1) + '%'}</p>
              <p className="text-sm font-bold">Buoy: {((aggregated.bojaCount / (aggregated.totalOvernights || 1)) * 100).toFixed(1) + '%'}</p>
            </div>
          </div>
          <div className="bg-white/60 p-3 rounded-lg shadow-sm border border-gray-100 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500">as captain: {captainTrips}</p>
            </div>
          </div>
        </div>
      </section>
