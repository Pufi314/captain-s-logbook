...
const Dashboard = ({ trips, onTripSelect }) => {
  ...
  const StatCard = ({ title, value, icon: Icon, subtitle, onClick }) => {
    return (
      <div onClick={onClick} className={`bg-white/70 md:bg-white/80 p-5 rounded-lg shadow-sm border border-gray-200 md:border-gray-100 flex items-center gap-4 ${onClick ? 'cursor-pointer hover:shadow-md transition-shadow text-left w-full' : ''}`} style={{ colorScheme: 'only light' }} {...(onClick ? { role: 'button', tabIndex: 0, onKeyDown: (e) => { if (e.key === 'Enter' || e.key === ' ') onClick(e); } } : {})}>
  ...
  return (
    <div className="space-y-6">
      <section>
        <h2 className="text-xl font-bold text-gray-800 md:text-white mb-4">General Statistics</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          <div className="bg-white/70 md:bg-white/80 p-5 rounded-lg shadow-sm border border-gray-200 md:border-gray-100 flex gap-4" style={{ colorScheme: 'only light' }}>
      ...
      <section>
        <h2 className="text-xl font-bold text-gray-800 md:text-white mb-4">Records</h2>
